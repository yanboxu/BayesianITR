...
model_fit <- function(model = 'ITR', tol_iter, thin, mh_sd,
  ...
  for(i in 1:dimI){
    ...
    Beta[i,] = mvrnorm(1,beta0,S0);
    sigma2_u[i] = exp(rnorm(1,mu_log_sd2_u0,sqrt(sigma_log_sd2_u0)));
    rho_u[i] = invlogit(rnorm(1,mu_logit_rho_u0,sqrt(sigma_logit_rho_u0)));
    
    # a{1-3},r, parameters for g function
    for(d in 1:dimD){
      a1[i,d] = rnorm(1,u10_f[d],sqrt(sd10_f));
      a2[i,d] = invlogit(rnorm(1,u20_f[d],sqrt(sd20_f)));
      a3[i,d] = invlogit(rnorm(1,u30_f[d],sqrt(sd30_f)));
      r[i,d] = rnorm(1,ur0_f[d],sqrt(sdr0_f));
      a = a1[i,d] * (exp(a2[i,d]*r[i,d]/2) - 1)/(exp(a2[i,d]*r[i,d]/2) + 1);
      b[i,d] = invlogit(rnorm(1,ub0_f[d],sqrt(sdb0_f))) * a;
    }
  }
  ...
  if(model == 'DPMITR'){
    ...
    for(d in 1:dimD){
      ...
      u3_f[1:K_f[d],d] = rnorm(K_f[d],u30_f[d],sqrt(sd30_f));
      ur_f[1:K_f[d],d] = rnorm(K_f[d],ur0_f[d],sqrt(sdr0_f));
      ub_f[1:K_f[d],d] = rnorm(K_f[d],ub0_f[d],sqrt(sdb0_f));
    ...
    for(i in 1:dimI){
      # Beta
      Beta[i,] = mvrnorm(1,Mu_b[C_u[i],],Sigma_b[C_u[i],,]); 
      
      # sigma2_u, rho_u
      sigma2_u[i] = exp(rnorm(1,mu_log_sd2_u[C_u[i]],sqrt(sigma_log_sd2_u)));
      rho_u[i] = invlogit(rnorm(1,mu_logit_rho_u[C_u[i]],sqrt(sigma_logit_rho_u)));
      ...
      # a{1-3},r, parameters for g function
      for(d in 1:dimD){
        a1[i,d] = rnorm(1,u1_f[C_f[i,d],d],sqrt(sd1_f));
        a2[i,d] = invlogit(rnorm(1,u2_f[C_f[i,d],d],sqrt(sd2_f)));
        a3[i,d] = invlogit(rnorm(1,u3_f[C_f[i,d],d],sqrt(sd3_f)));
        r[i,d] = rnorm(1,ur_f[C_f[i,d],d],sqrt(sdr_f));
        a = a1[i,d] * (exp(a2[i,d]*r[i,d]/2) - 1)/(exp(a2[i,d]*r[i,d]/2) + 1);
        b[i,d] = invlogit(rnorm(1,ub_f[C_f[i,d],d],sqrt(sdb_f))) * a;
      }
    }
  ...
  if(model == 'ITR'){
    modelFit = sampling_ind_C(input_vars,
                              ...
                              hyper_param_list,
                              param_list,
                              mvnfast::rmvn, mvnfast::dmvn, rinvgamma)
    modelFit['model'] = 'ITR'
  }
