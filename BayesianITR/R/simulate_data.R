...
simulate_data <- function(I, minT, maxT, unit, minDur, maxDur, minADur, maxADur, W,
  ...
  for(i in 1:I){
    Beta[i,] = MASS::mvrnorm(1,Mu_b[C_u[i],],diag(prior_sd2_cluster_sd2, P));
    sigma2_u[i] = exp(stats::rnorm(1,mu_log_sd2_u[C_u[i]],sqrt(prior_sd2_cluster_sd2)));
    rho_u[i] = invlogit(stats::rnorm(1,mu_logit_rho_u[C_u[i]],sqrt(prior_rho_cluster_sd2)));
    for(d in 1:D){
      a1[i,d] = stats::rnorm(1,u1_f[C_f[i,d],d],sqrt(prior_a1_cluster_sd2));
      a2[i,d] = invlogit(stats::rnorm(1,u2_f[C_f[i,d],d],sqrt(prior_rho_cluster_sd2)));
      a3[i,d] = invlogit(stats::rnorm(1,u3_f[C_f[i,d],d],sqrt(prior_rho_cluster_sd2)));
      r[i,d] = stats::rnorm(1,ur_f[C_f[i,d],d],sqrt(prior_r_cluster_sd2));
      a = a1[i,d] * (exp(a2[i,d]*r[i,d]/2) - 1)/(exp(a2[i,d]*r[i,d]/2) + 1);
      b[i,d] = invlogit(stats::rnorm(1,ub_f[C_f[i,d],d],sqrt(prior_rho_cluster_sd2))) * a;
    }
  }
  ...
  for(i in 1:I){
    ...
    U[i,1:J_I[i]] = U[i,1:J_I[i]] + MASS::mvrnorm(1, rep(0,J_I[i]), Kcov_u_i);
    Z[i,1:J_I[i]] = U[i,1:J_I[i]] + MASS::mvrnorm(1,rep(0,J_I[i]),diag(Sigma_e[i],J_I[i]));
  ...
  for(i in 1:I){
    ...
    for(l in 1:L_I[i]){
      ...
      if(d > 0){
        ...
        if(J_f == 1)
          Fnoise = stats::rnorm(1,0,sqrt(Kcov_f_i[idx_f,idx_f]))
        if(J_f > 1)
          Fnoise = MASS::mvrnorm(1,rep(0,J_f),Kcov_f_i[idx_f,idx_f])
